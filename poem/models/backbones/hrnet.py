...
from poem.external.metro.hrnet.config import config as hrnet_config
from poem.external.metro.hrnet.config import \
                    ...
                    for k in range(i - j):
                        if k == i - j - 1:
                            ...
                            conv3x3s.append(
                                nn.Sequential(
                                    nn.Conv2d(num_inchannels[j], num_outchannels_conv3x3, 3, 2, 1, bias=False),
                                    nn.BatchNorm2d(num_outchannels_conv3x3, momentum=BN_MOMENTUM), nn.ReLU(False)))
...
class HighResolutionNet(nn.Module):
    ...
    def _make_head(self, pre_stage_channels):
        ...
        final_layer = nn.Sequential(
            nn.Conv2d(in_channels=head_channels[3] * head_block.expansion,
                      ...
                      kernel_size=1,
                      stride=1,
                      padding=0), nn.BatchNorm2d(2048, momentum=BN_MOMENTUM), nn.ReLU(inplace=True))
        ...
        for i in range(num_branches_cur):
            if i < num_branches_pre:
                ...
                for j in range(i + 1 - num_branches_pre):
                    ...
                    conv3x3s.append(
                        nn.Sequential(nn.Conv2d(inchannels, outchannels, 3, 2, 1, bias=False),
                                      nn.BatchNorm2d(outchannels, momentum=BN_MOMENTUM), nn.ReLU(inplace=True)))
    ...
    def forward(self, x):
        # In this modified implementation, we have removed the cls head.
        # We will return the features from the last stage.
        x = self.conv1(x)
        x = self.bn1(x)
        x = self.relu(x)
        x = self.conv2(x)
    ...
    def init_weights(
        ...
        if os.path.isfile(pretrained):
            ...
            logger.info(f"HRNet has {param_size(self)}M parameters")
...
class HRNet(HighResolutionNet):
    ...
    def __init__(self, cfg):
        ...
        hrnet_yaml = "config/backbone/cls_hrnet_w40_sgd_lr5e-2_wd1e-4_bs32_x100.yaml"
        hrnet_update_config(hrnet_config, hrnet_yaml)
        
        super(HRNet, self).__init__(hrnet_config)
        self.init_weights(cfg.PRETRAINED)
        self.name = type(self).__name__
        

@BACKBONE.register_module()
class HRNetw60(HighResolutionNet):
    
    def __init__(self, cfg, **kwargs):
        hrnet_yaml = "config/backbone/cls_hrnet_w64_sgd_lr5e-2_wd1e-4_bs32_x100.yaml"
        hrnet_update_config(hrnet_config, hrnet_yaml)
